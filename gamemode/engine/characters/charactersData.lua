...
function classCharacterData.new( refOwner )
	...
	function public.addCharacter( newCharData )
		local totalChars = table.Count(characterList)
		for i = 1, totalChars do
			if characterList[i] == nil then
				public.setCharacter(i, newCharData)
				return
			end
		end
		public.setCharacter(totalChars, newCharData)
	end
	...
	if SERVER then 
		function public.toString()
			...
			compiledString = compiledString .. "&" .. selectedCharacter 
		...
		function public.loadFromString( compiledString )
			...
			local selectChar = tonumber(fateCharacterTable[3]) or 1
			...
			public.selectCharacter(math.Clamp(selectChar, 0, table.Count(characterList) - 1))
