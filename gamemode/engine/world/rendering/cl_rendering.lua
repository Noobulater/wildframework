...
function prepareEntAlpha( ent )
	...
	if ent:GetClass() == "prop_door_rotating" then 
		...
		if vDistance > -80 then	
			if vDistance > 80 then	
				if util.within2DBox(ent:LocalToWorld(min), ent:LocalToWorld(max), LocalPlayer():GetPos()) then
					alpha = 40
				end		
			else
...
function renderThink()
	if world.data == nil then 
		...
		renderDistance = GAMEMODE.RenderDistance or 1000
				...
				if distance < renderDistance then
					if !IsValid(entry.getEnt()) then
						...
						if distance < solidDist && (entry.cd == nil || entry.cd <= CurTime()) then 
							--timer.Simple((LocalPlayer():Ping() * 2.5)/renderDistance, function() entry.getEnt():SetNoDraw(true) end) 
