...
local function createCombatMode()
		...
		if SERVER then
			if boolVictory then
				...
				for key, ply in pairs(util.getAlivePlayers()) do
					...
					ply.claimReward = function() 
						local item = classItemData.genItem(reward)
						ply:ChatPrint("Congratulations, you have won a ".. item.getName() .. " for killing all zombies")
	...
	if CLIENT then
		mode.paint = function()
			paintText("Escape... If you can!", "zgEffectText", ScrW()/2, 25, Color(255,255,255,155), true, true)
		end
	end
	return mode 
end
...
local function generate()
	...
	gameData.setMode(1, combatMode)
	gameData.setMode(2, gameOverMode)
